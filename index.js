...
function replaceText(word, replacement, text) {
  ...
  // Використання методу `replace` для заміни слова на фразу у тексті.
...
function checkWord(word, text) {
  ...
  // Використання методу `test` для перевірки наявності слова у тексті
...
function findWordOccurrences(str, word) {
  ...
  // Створюємо пустий масив matches для зберігання індексів входжень
  let matches = [];
  let match;

  // За допомогою циклу while шукаємо всі входження слова в рядок
  ...
  // Повертаємо масив з індексами входжень слова у рядок
  return matches;
}
...
function checkRegexFlags(regex) {
  ...
  // Перевіряємо наявність флагів 'g' та 'm' за допомогою методу `includes`
  let hasGlobalFlag = flags.includes("g");
  let hasMultilineFlag = flags.includes("m");
...
function checkFlags(regex) {
  ...
  // Створюємо масив для зберігання використаних флагів та самого виразу.
  let flagsAndPattern = [];

  // Перевіряємо, чи використовується флаг 'i' (ignoreCase) у регулярному виразі.
  if (regex.ignoreCase) {
    flagsAndPattern.push("ignoreCase");
  }

  // Витягуємо сам рядок виразу.
  let pattern = regex.toString().match(/\/(.*?)\//)[1];
  // Додаємо цей вираз до масиву.
  flagsAndPattern.push(pattern);

  // Повертаємо масив використаних флагів та самого виразу.
  return flagsAndPattern;
}
